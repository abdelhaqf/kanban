<template lang="html">
  <div class="modal" v-show="isVisible">
    ...
    <div class="container">
      ...
      <div class="header">
        <p-check style="margin: 4px 4px" @change="updateTitlePriority" v-model="canEditUpdate" class="p-switch p-fill" color="success">Edit</p-check>
      ...
      <div class="content">
        <table>
          ...
          <tr>
            ...
            <td v-if="canEditUpdate">
              <star-rating  v-model="oldCardData.priority" :padding="10" :border-color="'#4C3300'" :border-width="2" :show-rating="false" :star-size="25" />
            </td>
            <td v-else><star-rating :read-only="true" v-model="curCardData.priority" :padding="10" :border-width="2" :show-rating="false" :star-size="25" /></td>
          </tr>
          <tr>
            <td>due</td>
            <td v-if="canEditUpdate">
      ...
      <div class="footer">
        <div class="" style="width: 100%;display: flex; ">
          <input ref="myupdate" v-model="newUpdateData.text" style="flex-grow: 99;">
          <button @click="confirmAddUpdate">update</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    updateTitlePriority() {
      ...
      // this.canEditUpdate=!this.canEditUpdate
    },
    confirmAddUpdate() {
      this.curCardData = this.findCard(this.curCardData)
      for(var k in this.curCardData) this.oldCardData[k]=this.curCardData[k];
