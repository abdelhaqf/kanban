<template lang="html">
  ...
  <div class="modal" v-show="isVisible">
    <div class="container">
      <div class="header">
        <p-check style="margin: 4px 4px"
                 @change="updateTitlePriority"
                 v-model="canEditUpdate"
                 class="p-switch p-fill"
                 color="success"><b>Toggle Edit</b></p-check>
      ...
      <div class="content">
        <table>
          ...
          <tr>
            ...
            <td v-if="canEditUpdate">
              <star-rating v-model="oldCardData.priority"
                           :padding="10"
                           :border-color="'#4C3300'"
                           :border-width="2"
                           :show-rating="false"
      ...
      <div class="help">
        <drag-verify v-if="resetDeleteButton"
                     @passcallback="deleteCard"
                     :width="400"
                     :height="40"
...
<script>
  ...
  export default {
    name: 'modal',
    components: { dragVerify, StarRating },
    data: () => {
      return {
        isVisible: false,
        canEditUpdate: false,
        newUpdateData: { text: '', created: '', type: 'normal' },
    ...
    methods: {
          ...
          if (this.curCardData.due != this.oldCardData.due) {
            ...
            this.newUpdateData = {
              text:
                ...
                this.$options.filters.myTime(this.curCardData.due) +
                ' to ' +
                this.$options.filters.myTime(this.oldCardData.due),
              created: new Date(),
              type: 'system'
            }
            this.curCardData.updates.unshift(this.newUpdateData)
      ...
      findCard(x) {
        ...
        for (var key in this.kanbanGroups) {
          if (
            (temp = this.kanbanGroups[key].find(e => {
              return e.created == x.created
            }))
          )
            return temp
        }
