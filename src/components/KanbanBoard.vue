  ...
  <div class="kanbanBoard">
    <div class="nav">
      <div class="departments">
        <p-radio style="font-size: 0.8em;"
                 @change="changeDept(dept)"
                 :value="dept"
                 v-model="DeptSelector"
                 v-for="dept in curUser.department"
                 class="p-icon p-round p-smooth p-plain"
                 color="success-o">
          <menu-icon class="icon mdi mdi-check" slot="extra" /> {{dept}}
        </p-radio>
      </div>
    ...
    <div class="kanban-container">
      <div v-for="(item,idx) in kanbanGroups" class="kanban-group" :style="{backgroundColor: colors[idx]}">
        ...
        <draggable class="mydraggable"
                   v-model="kanbanGroups[idx]"
                   v-bind="dragOptions"
                   group="kanban"
                   @start="drag=true"
                   @end="drag=false"
                   @change="updateGroup">
          <KanbanCard v-for="element in kanbanGroups[idx]"
                      :key="element.created"
                      :kanban="element"
                      @update="$refs.myupdatemodal.open(element)"></KanbanCard>
        </draggable>
      </div>
  ...
  export default {
      ...
      updateGroup(x) {
        if (Object.keys(x)[0] == 'added') {
          ...
          for (var key in this.kanbanGroups) {
            if (
              this.kanbanGroups[key].find(e => {
                return e == x.added.element
              })
            ) {
              tmp = key
          ...
          this.newUpdateData = { text: 'moved to ' + tmp, created: new Date(), type: 'system' }
          x.added.element.updates.unshift(this.newUpdateData)
          this.newUpdateData = { text: '', created: '', type: 'normal' }
        } else {
    ...
    data: function() {
      return {
        ...
        kanbanGroups: { backlog: [], todo: [], hold: [], process: [], done: [], archive: [] },
        colors: {
          backlog: '#253858',
          todo: '#00B8D9',
          hold: '#FF5630',
